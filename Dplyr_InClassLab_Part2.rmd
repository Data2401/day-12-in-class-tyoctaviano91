...
my_flights_piped <- my_flights %>% 
  mutate(.before = 1, gain = dep_delay - arr_delay, speed = distance / air_time * 60) %>% 
  arrange(desc(gain))
```
...
my_flights %>% 
  ...
  summarize(across(air_time, list(mean = mean, max = max, min = min)))
...
my_flights %>% filter(dest %in% c("HOU", "IAH")) %>% 
  group_by(origin) %>% 
  summarise(mean_delay = mean(arr_delay, no.rm = T)) %>% arrange(mean_delay)
...
my_flights %>% 
  ...
  select(origin, dest, carrier, arr-delay, everything())
...
```{r}
starwars %>% filter(eye_color = "brown") %>% summarise(mean_height = mean(height))
...
```{r}
starwars %>% drop_na(height) %>% group_by(eye_color) %>% summarise(mean_height = mean(height))
